...
def main():
    ...
    model_path = params['GAMENet_model_path']
    ...
    glu_encoder = RNNEncoder(1, emb_dim // 4, emb_dim)
    model = GAMENet(lab_dim=len(data_test[1][0]), med_dim=len(data_test[0][3][0]), ehr_adj=ehr_adj, ddi_adj=ddi_adj,
                    glu_encoder=glu_encoder, emb_dim = emb_dim, device=device, ddi_in_memory=DDI_IN_MEM)
