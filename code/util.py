...
class DealDataset(Dataset):
    ...
    def __init__(self, wholedataset):
        super(DealDataset).__init__()
        self.dataset = wholedataset
        self.len = len(wholedataset)
...
def CreateDataLoader(dataset, batch_size = 32):
    ...
    # dataset_wrapper = DealDataset(dataset)
    train_loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, collate_fn=custom_collate)
...
def ddi_rate_score(record, ddi_A):
    # ddi rate
    all_score = np.zeros((12,12))
    all_cnt = 0
    for patient in record:

        all_cnt += np.sum(patient)
        patient = np.array([patient])
        vec = np.matmul(np.transpose(patient), patient)
        all_score += vec * ddi_A

    if all_cnt == 0:
        return 0
    return np.sum(all_score) / 2 /  all_cnt
