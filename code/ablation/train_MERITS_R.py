...
import sys
sys.path.append("../")
sys.path.append("/home/LAB/zhuqs/GAMENet/code")
...
def eval(model, data_eval, ddi_adj):
    # evaluate
    print('')
    model.eval()
    smm_record = []
    ...
    for i in range(int(len(data_eval) / batch_size)):
        ...
        for k in range(len(seq_input)):
            if seq_input[k][3][-1] == seq_input[k][3][-2]:
                continue
...
def main():
    ...
    glu_encoder = TimeLSTM(1, 64, cuda_flag = torch.cuda.is_available())
    model = MERITS(lab_dim=len(data[1][0]), med_dim=len(data[0][3][0]), ehr_adj=ehr_adj, ddi_adj=ddi_adj, glu_encoder=glu_encoder, time_adj = time_adj, emb_dim=64, device=device, ddi_in_memory=DDI_IN_MEM, glu_dim = 64)
    ...
    for train_index, test_index in kf.split(data):
        ...
        while epoch < Total_epoch :
            ...
            T *= decay_weight
            ddi_rate, ja, prauc, avg_p, avg_r, avg_f1 = eval(model, data_eval, ddi_adj)
